 ...
 * 
 * @param {number[]} candyType - Numbers denoting types of candy
 * @returns {number} - Returns the max number of different candy to be eaten
 */
function distributeCandies (candyType) {
  let unique = new Set(candyType);
  return Math.min(unique.size, Math.floor(candyType.length / 2));
}