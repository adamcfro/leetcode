def slowestKey(releaseTimes, keysPressed):
    ...
    n keys, one at a time.You are given a string keysPressed of length n, where 
    ...
    Args:
        releaseTimes (list[int]): A list of integers representing duration.
        keysPressed (str): A string of lowercase English letters.

    Returns:
        str: Returns the keypress with the longest duration.
    """
    pressLength = releaseTimes[0]
    index = 0
    for i in range(1, len(keysPressed)):
        ...
        if temp > pressLength:
            pressLength = temp
            index = i
        elif temp == pressLength:
            if ord(keysPressed[i]) > ord(keysPressed[index]):
                pressLength = temp
                index = i
