 ...
 *       range [i + 1, n - 1] by 1, ensuring their percentage never goes below
 *       0, i.e, batteryPercentages[j] = max(0, batteryPercentages[j] - 1).
 *     • Move to the next device.
 *   Otherwise, move to the next device without performing any test.
 *
 * Returns an integer denoting the number of devices that will be tested after
 * performing the test operations in order.
 *
 * @format
 * @param {number[]} batteryPercentages - An array of number representing battery percentages
 * @returns {number} - Returns a count of devices with a battery percentage greater than 0
 */

function countTestedDevices(batteryPercentages) {
  let testedDevices = 0;
  let increment = 0;
  batteryPercentages.forEach((element) => {
